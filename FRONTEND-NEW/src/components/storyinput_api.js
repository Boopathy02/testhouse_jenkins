...
const DEFAULT_TEST_TYPES = ["ui"];

const StoryInputApi = ({ onBack, onNext, testCases, setTestCases, projectName,
  projectId, }) => {
  const [userStoriesInput, setUserStoriesInput] = useState("");
  const [selectedFile, setSelectedFile] = useState(null);
  const selectedTestTypes = DEFAULT_TEST_TYPES;
  ...
  const [stepDefinitionContent, setStepDefinitionContent] = useState("");
  ...
  const getStorageKey = (suffix) => {
    ...
    return `testify:${id}:story-api:${suffix}`;
  };
  ...
  useEffect(() => {
    try {
      ...
      if (saved) {
        ...
        if (typeof parsed.featureContent === "string") {
          setFeatureContent(parsed.featureContent);
        }
        if (typeof parsed.stepDefinitionContent === "string") {
          setStepDefinitionContent(parsed.stepDefinitionContent);
        }
        if (typeof parsed.showGeneratedFiles === "boolean") {
          setShowGeneratedFiles(parsed.showGeneratedFiles);
        }
        if (typeof parsed.generatedFilesError === "string") {
          setGeneratedFilesError(parsed.generatedFilesError);
        }
      }
  ...
  const ensureActiveProject = useCallback(async () => {
    ...
    if (activeProjectId && (!name || storedName === name)) {
  ...
  const fetchGeneratedApiFiles = useCallback(async () => {
    ...
    try {
      ...
      const featureRes = await axios.get(
        `${API_BASE_URL}/projects/${activeId}/files/content`,
        { params: { path: "tests/api_test/user_story.feature" }, headers }
      );
      const stepRes = await axios.get(
        `${API_BASE_URL}/projects/${activeId}/files/content`,
        { params: { path: "tests/api_test/steps/step_definition.py" }, headers }
      );
      setFeatureContent(featureRes.data?.content || "");
      const rawSteps = stepRes.data?.content || "";
      const cleanedSteps = rawSteps
        ...
        .filter((line) => {
          ...
          if (trimmed.startsWith("use_step_matcher")) return false;
          return true;
        })
        .join("\n");
      setStepDefinitionContent(cleanedSteps.trim() ? cleanedSteps : rawSteps);
    } catch (err) {
      const detail = err?.response?.data?.detail || err?.message || "Failed to load generated files.";
      setGeneratedFilesError(detail);
      setFeatureContent("");
      setStepDefinitionContent("");
    } finally {
      ...
      if (selectedFile) {
        ...
        const stories = storyObjectsToUse
          ? storyObjectsToUse
              ...
              .map((s) => s.trim())
              .filter((s) => s.length > 0)
          : [userStoriesInput.trim()].filter((s) => s.length > 0);
  ...
  return (
    <div className={styles.storyInputContainer}>
      ...
      <div className={styles.contentBox}>
        <h3 className={styles.title}>Import API User Stories</h3>
        <p>Add API user stories from Jira, Excel, or create them manually</p>
        ...
        <textarea
          rows="5"
          cols="60"
          placeholder="Paste your API Gherkin feature here (kept exactly as-is)"
        ...
        {showGeneratedFiles && (
          <div className={styles.generatedFilesGrid}>
            <div className={styles.testCaseCard}>
              <div className={styles.testCaseHeader}>
                <h4 className={styles.testCaseTitle}>user_story.feature</h4>
              </div>
              {loadingGeneratedFiles ? (
                <p>Loading generated files...</p>
              ) : generatedFilesError ? (
                <p className={styles.errorText}>{generatedFilesError}</p>
              ) : (
                <div className={styles.code}>
                  <pre>
                    <code>{featureContent || "No user_story.feature found."}</code>
                  </pre>
                </div>
              )}
            </div>
            <div className={styles.testCaseCard}>
              <div className={styles.testCaseHeader}>
                <h4 className={styles.testCaseTitle}>step_definition.py</h4>
              </div>
              {loadingGeneratedFiles ? (
                <p>Loading generated files...</p>
              ) : generatedFilesError ? (
                ...
                <div className={styles.code}>
                  <pre>
                    <code>{stepDefinitionContent || "No step_definition.py found."}</code>
                  </pre>
                </div>
              )}
            </div>
